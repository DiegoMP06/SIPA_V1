    ...
    return (
        <form className={`flex ${className}`} onSubmit={handleSearch} {...props}>
            <input
                ...
                name="search"
                id="search"
                className="flex-1 block rounded-tl rounded-bl border-gray-400 shadow-sm focus:border-rose-500 focus:ring focus:ring-rose-300 focus:ring-opacity-50"
                placeholder={placeholder}
                onChange={(e) => setData({search: e.target.value, page: 1})}
                value={data.search}
            />

            <button type="submit" disabled={processing} className="bg-rose-700 text-white px-4 py-2 rounded-tr rounded-br hover:bg-rose-600 transition-colors disabled:opacity-25 disabled:bg-rose-700 disabled:cursor-default cursor-pointer">
