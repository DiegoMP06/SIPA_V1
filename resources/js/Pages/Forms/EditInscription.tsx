...
export default function EditInscription({specialties, shifts, period, pay} :  EditInscriptionProps) {
    const [alerts, setAlerts] = useState<string[]>([]);

    const {data, setData, patch, errors, processing} = useForm<PayType>(pay);
    ...
    const handleSubmit : FormEventHandler = (e) => {
        ...
        if(currentAlerts.length === 0) {
            patch(route('inscription.update', {pay}), {
                preserveScroll: true,
            })
        }
    }
        ...
        <FormLayout title="Editar Ficha de Inscripción" period={period}>
            ...
            <div className="my-16 px-4 max-w-4xl mx-auto">
                <form className="bg-white shadow p-6 grid grid-cols-1 gap-6" onSubmit={handleSubmit}>
                    ...
                    <SubmitButton value="Guardar Cambios" disabled={processing} />
                </form>
            </div>
        </FormLayout>
    );
