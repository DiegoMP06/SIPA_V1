...
export default function ShowPay({period, pays, typePay} : ShowPayProps) {
    ...
    return (
        <FormLayout title="Fichas de Pago" period={period}>
            ...
            { pays.map(pay => (
                    <div key={pay.id} className="bg-white rounded-2xl p-6 shadow-md border border-rose-200 space-y-6">
                        <h2 className="text-gray-800 text-2xl font-bold">
                            { pay.name } { pay.father_last_name } { pay.mother_last_name }
                        </h2>

                        <div className="space-y-2 divide-y divide-gray-300">
                            <p className="text-rose-700 font-bold text-lg flex justify-between p-2 gap-4">
                                Concepto de Pago: { '' }
                        ...
                        <nav className="my-16 flex justify-between gap-4">
                            <a href={route(nameRoute + '.show', {pay})} className="text-white font-bold px-4 py-2 bg-rose-700 hover:bg-rose-600 transition-colors inline-block uppercase" target="_blank">
                                Ver Ficha
                            </a>

                            <Link href={route(nameRoute + '.edit', {pay})} className="text-rose-800 font-bold px-4 py-2 bg-rose-100 hover:bg-rose-200 transition-colors inline-block uppercase">
                                Modificar Ficha
                            </Link>
                        </nav>
                    </div>
