...
import { PageProps, PaginateProps, TypePayType } from "@/types";
...
type TypePaysProps = {
    typePays: PaginateProps<TypePayType>;
};

export default function TypePays({ auth, typePays }: PageProps<TypePaysProps>) {
    const hasTypePays = useMemo(() => typePays.data.length > 0, [typePays]);
    const { handleActive } = useTypePays();

    return (
        <AuthenticatedLayout user={auth.user} header="Tipos de Pago">
            <Head title="Tipos de Pago" />

            <NavigateLink name="type-pays.create">
                Agregar Tipo de Pago
            </NavigateLink>

            {hasTypePays ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
                    {typePays.data.map((typePay) => (
                        <div
                            key={typePay.id}
                        ...
                        >
                            <div>
                                ...
                                <p className="text-gray-800 font-bold">
                                    Estado:{" "}
                                    {typePay.active ? "Activo" : "Inactivo"}
                                </p>
                            </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <Link
                                    title="Editar"
                                    href={route("type-pays.edit", { typePay })}
            ...
            <Pagination pagination={typePays} />
        </AuthenticatedLayout>
    );
}
