            ...
            { hasSubjects ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
                    { subjects.data.map(subject => (
                        <div key={subject.id} className="bg-white shadow-md hover:shadow-xl transition-shadow duration-300 rounded-2xl p-5 flex flex-col gap-4 justify-between border border-rose-200">
                            <Link href={route('subjects.show', { subject })} className="block text-xl font-bold text-rose-700 uppercase">
                                { subject.subject }
                            </Link>

                            <div className="flex gap-3">
                                <button title={subject.active ? 'Desactivar' : 'Activar' } type="button" className="font-bold transition-colors text-center bg-rose-700 text-white p-2 hover:bg-rose-600" onClick={() => handleActive(subject)}>
                                    { subject.active ? (
                                ...
                                <Link title="Editar" href={route('subjects.edit', { subject })} className="font-bold transition-colors text-center bg-rose-500 text-white p-2 hover:bg-rose-600">
                                ...
                                <button title="Eliminar" type="button" onClick={() => handleDelete(subject.id)} className="font-bold transition-colors text-center bg-rose-100 text-rose-800 p-2 hover:bg-rose-200">
