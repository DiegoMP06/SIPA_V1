...
export default function usesubjects({
    ...
    const activeSemester = (open: boolean, id: SemesterType["id"]) => {
        if (!open) {
            const updatedClassroms = data.classrooms?.filter(
                (classroom) => classroom.semester_id !== id,
            );
            setData("classrooms", updatedClassroms);
        }

        const semestersUpdated = semestersOptions.map((semester) =>
            semester.id === id ? { ...semester, open } : semester,
        );
    ...
    const addSpecialty = (
        ...
        const classroomFind = data.classrooms.find(
            (classroom) =>
                classroom.specialty_id === specialty_id &&
                classroom.semester_id === semester_id,
        );

        if (classroomFind) {
            const classroomsUpdated = data.classrooms.filter(
                (classroom) => classroom.id !== classroomFind.id,
            );
    ...
    const validate = () => {
        ...
        if (!data.id && data.classrooms?.length === 0) {
            currentAlerts = [...currentAlerts, "Debe agregar al menos un aula"];
        }
    ...
    const handleActive = (subject: SubjectType) => {
        Swal.fire({
            title: "Atencion",
            text: subject.active
                ? "¿Desea desactivar la materia?"
                : "¿Desea activar la materia?",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                router.patch(
                    ...
                    { ...subject, active: !subject.active, dashboard: true },
                    {
                        preserveScroll: true,
                        onSuccess() {
                            Swal.fire({
    ...
    const isOpen = useMemo(
        () => (semester: SemesterOptionType) => semester.open,
        [semestersOptions],
    );
