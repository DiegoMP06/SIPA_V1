...
export default function useTeachers() {
    ...
    const validate = () => {
        ...
        if (
            Object.values(data).some((value) => value?.toString().trim() === "")
        ) {
            currentAlerts = [
                ...currentAlerts,
    ...
    const handleActive = (teacher: TeacherType) => {
        Swal.fire({
            title: "Atencion",
            text: teacher.active
                ? "¿Desea desactivar el profesor?"
                : "¿Desea activar el profesor?",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                router.patch(
                    route("teachers.update", { teacher }),
                    { ...teacher, active: !teacher.active, dashboard: true },
                    {
                        preserveScroll: true,
                        onSuccess() {
                            Swal.fire({
